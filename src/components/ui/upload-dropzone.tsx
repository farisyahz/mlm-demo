    ...
    >
      ...
      >
        ...
        <div className="mt-3 space-y-1 text-center">
          <p className="text-sm font-semibold">Seret dan lepas file di sini</p>
            ...
            {typeof description === 'string' ? (
              ...
              <>
                {description?.maxFiles &&
                  `Anda dapat mengunggah ${description.maxFiles} file.`}{' '}
                {description?.maxFileSize &&
                  `Maksimal ${description.maxFileSize}.`}{' '}
                {description?.fileTypes && `Format yang diterima: ${description.fileTypes}.`}
              </>
            )}
          </p>
        </div>
      ...
      {isDragActive && (
        <div className="pointer-events-none absolute inset-0 rounded-lg">
          <div className="dark:bg-accent/40 bg-accent flex size-full flex-col items-center justify-center rounded-lg">
            ...
            <p className="mt-3 text-sm font-semibold">Lepas file di sini</p>
          </div>
        </div>
      )}
    </div>
